package counter;
import commandpattern.DecrementCounterCommand;
import commandpattern.HistoryList;
import commandpattern.IncrementCounterCommand;
import observerpattern.Counter;
import observerpattern.OvalFrame;
import observerpattern.RectFrame;
import observerpattern.TextFrame;
import statepattern.CounterState;
import statepattern.SingleDigit;
...
public class JFrameCounter extends JFrame {
    ...
    private void jButtonIncrement_actionPerformed(ActionEvent e) {
        IncrementCounterCommand ic = new IncrementCounterCommand(counter);
        ic.execute();
        historyList.addCommand(ic);
      //counter.increment();
    }

    private void jButtondecrement_actionPerformed(ActionEvent e) {
        DecrementCounterCommand dc = new DecrementCounterCommand(counter);
        dc.execute();
        historyList.addCommand(dc);
    	//counter.decrement();
    }
